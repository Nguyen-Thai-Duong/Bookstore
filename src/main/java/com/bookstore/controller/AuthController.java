...
public class AuthController {
    ...
    public String login(@RequestParam String email,
        ...
        if (normalizedEmail.length() > 150) {
            redirectAttributes.addAttribute("error", "Email quá dài");
            return "redirect:/login";
        }
        if (normalizedPassword.length() > 72) {
            redirectAttributes.addAttribute("error", "Mật khẩu quá dài");
            return "redirect:/login";
        }
        ...
        if (user == null) {
            redirectAttributes.addAttribute("error", "Sai email hoặc mật khẩu");
            return "redirect:/login";
        }
    ...
    public String register(@RequestParam String fullName,
        ...
        if (normalizedName.length() > 100) {
            redirectAttributes.addAttribute("error", "Họ và tên quá dài");
        ...
        if (!normalizedAddress.isEmpty() && normalizedAddress.length() > 255) {
            redirectAttributes.addAttribute("error", "Địa chỉ quá dài");
            return "redirect:/register";
        }
        ...
        if (!normalizedPhone.isEmpty() && !normalizedPhone.matches("\\d{8,15}")) {
            redirectAttributes.addAttribute("error", "Số điện thoại phải từ 10-11 chữ số");
            return "redirect:/register";
        }
